...
#define INDEX(c) ((int)c - (int)'a')char** words; 
int length = 0; 
int letter[16]; 
int scanned[16]; 
...
int check(int length) {
	/*checks in what range of letters it should look for words depends on information if any key was pressed*/
	if (scanned[length] != -1)return number[scanned[length]]; 
	else
	return 29; 
	}
...
int main(){
	...
	/* Construct trie*/
	for (int i = 0;  i < size;  i++)enter(root, words[i]); 
	free(words); 
	temp = root; 

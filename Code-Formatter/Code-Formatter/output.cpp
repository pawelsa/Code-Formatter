...
int check(int length) {
	/*checks in what range of letters it should look 
	for words depends on in
	formation if any key was pressed*/
	if (scanned[length] != -1)return number[scanned[length]];
	else return 29;
	}

int look_
for(DICTIONARY *temp, int a) {
	if (!temp)return NULL;
	letter[length] = a;
	length++;
	if (temp->word_end == 1) {
...
int main(){
	...
	for (int i = check(length); i < number[scanned[length] + 1]; i++, j++)look_
	for(temp->word[number[scanned[length]] + j], (number[scanned[length]] + j));
	return 0;
	}
